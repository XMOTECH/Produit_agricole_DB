...
const Stocks: React.FC = () => {
    const [activeTab, setActiveTab] = useState<string>('catalogue'); // 'catalogue' ou 'perte'
    ...
    // --- FORMS STATES ---
    const [newProduit, setNewProduit] = useState<string>('');
    const [newVariete, setNewVariete] = useState<{ nom: string; description: string; id_produit: string }>({ nom: '', description: '', id_produit: '' });
    const [newPerte, setNewPerte] = useState<{ id_variete: string; qte_kg: string; motif: string }>({ id_variete: '', qte_kg: '', motif: '' });
    ...
    const handleCreateVariete = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await addVariete({
    ...
    const handleDeclarerPerte = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await addPerte({
    ...
    const styles = {
        ...
        button: (color: string): React.CSSProperties => ({
        ...
        alert: (type: string): React.CSSProperties => ({
