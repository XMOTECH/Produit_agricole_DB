...
const ActivityChart: React.FC<ActivityChartProps> = ({ data }) => {
    ...
    const formatDate = (dateStr: string) => {
        ...
        return isNaN(d.getTime()) ? dateStr : new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: '2-digit' }).format(d);
    };
        ...
        if (active && payload && payload.length) {
            return (
                <div style={{ backgroundColor: 'white', border: '1px solid #e2e8f0', padding: '15px', borderRadius: '8px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
                    <p style={{ fontWeight: 'bold', marginBottom: '8px', color: '#1e293b' }}>{new Date(label as string | number).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
