...
import { LayoutDashboard, Sprout, ShoppingCart, AlertTriangle, LogOut } from 'lucide-react';
...
const Sidebar: React.FC = () => {
    ...
    const menuItems = [
        ...(isSeller ? [{ name: 'Tableau de Bord', icon: LayoutDashboard, path: '/' }] : []),
        ...(isSeller ? [{ name: 'Opérations (Ventes)', icon: ShoppingCart, path: '/operations' }] : []),
        { name: 'Catalogue & Achats', icon: Sprout, path: '/stocks' },
        { name: 'Alertes / Tendances', icon: AlertTriangle, path: '/alertes' },
    ];
    ...
    return (
        <div style={{ width: '260px', backgroundColor: '#1e293b', height: '100vh', padding: '20px', position: 'fixed', left: 0, top: 0, color: 'white' }}>
            ...
            {/* Footer Sidebar */}
            <div style={{ position: 'absolute', bottom: '20px', left: '20px', right: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ color: '#475569', fontSize: '0.75rem', textAlign: 'center' }}>
                    © 2026 Méñenté Capital v2.0
                </div>
            </div>
        </div>
    );
